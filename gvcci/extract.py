...
from . import htmlpreview
from . import scoring
from . import colorgenerator
...
def main():
    ...
    no_examples_html = args['no_examples_html']
    symlink_wallpaper = args['symlink_wallpaper']

    with open(resource('resources/gvcci-title-ascii.txt'), 'r') as logo:
        log(logo.read())
    ...
    for img_file_path in image_paths:
        ...
        improved_centers = clustering.hhsl_cluster_centers_as_hsl(hsl_colors)

        dominant_dark_and_light_colors = scoring.find_dominant_by_frequency(
            hsl_colors)
        ...
        reference_dominant_light_color = np.array(
            [[0, 0, min(max(dominant_light[0][2], 0.8), 0.95)]])
        ...
        if background == "dark" or (background == "auto" and bg_color[0][2] < 0.5):
            ...
            dominant_light = colorgenerator.correct_saturation(dominant_light)
            bg_color = dominant_light
            fg_color = dominant_dark
        elif background[0] == "#":
            bg_color = convertershex2rgb(background)
            bg_color = hasel.rgb2hsl(
                np.array(bg_color).reshape(1, 1, 3)).reshape(1, 3)
            if (bg_color[0][2] < 0.5):
                fg_color = dominant_light
            else:
                fg_color = dominant_dark
        ...
        ansi_colors_unconstrained = scoring.pick_n_best_colors(
            8, improved_centers, dominant_dark, dominant_light, min_dark_contrast, min_light_contrast)
        ansi_colors_normal = scoring.clip_between_boundaries(
            ansi_colors_unconstrained, dominant_dark, dominant_light, min_dark_contrast, min_light_contrast)
        ansi_colors_sorted = scoring.sort_colors_by_closest_counterpart(
            ansi_colors_normal, standard_ansi_colors)
        # ansi_colors_sorted = ansi_colors_normal # already sorted when you run it through pick_n_best_with_ref
        ansi_colors_normal_and_bright = colorgenerator.generate_complementary(
            ansi_colors_sorted)
        ...
        if not no_examples_html:
            html_contents += htmlpreview.get_html_contents(np.vstack(
                (black, black_bright, ansi_colors)), bg_color, fg_color, img_file_path)
            ...
            with open("examples.html", "w") as result_file:
        ...
        colors = {'wallpaper': img_file_path}
        for name, hsl in colors_hsl.items():
            rgb = converters.hsl2rgb(hsl)
            hex = converters.hsl2hex(hsl)
        ...
        log("=========== Terminal Colors ===========")
        with open(resource('templates/columns-with-headers.txt'),
                  'r') as print_template:
        ...
        output_dir_path = os.path.join(
            os.path.expanduser("~/.gvcci/themes"), image_name)
        try:
        ...
        if (os.path.isfile(template_file_or_dir_path)):
            ...
            with open(resource(template_file_path), 'r') as template_file:
                template = template_file.read()
                template_file_name = os.path.basename(template_file_path)

                output_theme_path = os.path.join(
                    output_dir_path, template_file_name)
            ...
            for template_file in os.listdir(template_file_or_dir_path):
                template_file_path = os.path.join(
                    template_file_or_dir_path, template_file)
                with open(resource(template_file_path), 'r') as template_file:
                    template = template_file.read()
                    template_file_name = os.path.basename(template_file_path)
