...
def hex_codes_to_html_list(hex_codes, hsl_colors):
    ...
    for i in range(len(hex_codes)):
        html += "<li style='height: 24px; overflow: hidden; color: " + \
            hex_codes[i] + "'>"
        html += "<p style='font-family: monospace; whitespace: no-wrap; margin-top: -5px; font-size: 18px;'>def a = " + \
            str((100 * np.clip(hsl_colors[i], 0, 1)).astype(int)) + "</p>"
        html += "</li>\n"
    return html + "</ul>\n"


def hsl_color_list_to_html_list(color_list):
    hex_codes = converters.hsllist2hex(color_list)
...
def get_preview_image(img_file_path, ansi_colors, bg_color, fg_color):
    hex = converters.hsllist2hex(np.vstack((ansi_colors, fg_color, fg_color)))
    bg_rgb = converters.hsl2rgb(bg_color)
...
White"""
    ...
    color_groups = [
        ...
        (fg, ["toListRecursive", "toList", "colorscheme", "example", "_",
              "this", "head", "tail(", "tail)", " go", "stream", "reverse"]),
        (red, ["= ", "=>"]),
    ...
    html = "<img src='" + img_file_path + "' style='object-fit: cover; height: " + \
        str(height) + "px; width: " + str(width) + \
        "px; position: absolute; top: 0; left: 0;'/>"
    html += "<div style='z-index: 10; padding: " + str(text_padding) + "px; position: absolute; top: " + str(terminal_padding) + "px; left: " + str(terminal_padding) + "px;right: " + str(
        terminal_padding) + "px;bottom: " + str(terminal_padding) + "px; background-color: rgba(" + str(bg_rgb[0]) + ", " + str(bg_rgb[1]) + ", " + str(bg_rgb[2]) + ", 1.0)'><pre style='margin: 0;'>"

    for group in color_groups:
        if (group[0] == white or group[0] == black):
            for word in group[1]:
                sample = sample.replace(word, wrap_in_span(
                    word, hex[group[0]], hex[black] if group[0] == white else hex[white]))
        else:
            for word in group[1]:
                sample = sample.replace(
                    word, wrap_in_span(word, hex[group[0]]))
    ...
    img_preview = "<img src='" + img_file_path + "' style='margin-bottom: 40px; object-fit: contain; height: " + \
        str(height) + "px; width: " + str(width) + "px;'/>"
